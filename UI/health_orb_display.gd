...
func _ready():
	...
	while i < max_health_amt:
		var health_orb_instance : HealthOrb = HEALTH_ORB_SCENE.instantiate()
		add_child(health_orb_instance)
		health_orb_instance.emit_particles = emit_particles
		i += 1
...
func update_health_orbs():
	var i = 0.0
	while i <= max_health_amt - 1.0:
		var health_orb = get_child(i)
		if health_orb:
